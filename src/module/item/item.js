...
export class ItemSFRPG extends Mix(Item).with(ItemActivationMixin, ItemCapacityMixin) {
    ...
    async getChatData() {
        ...
        if (data.hasOwnProperty("activation")) {
            ...
            if (data.altRange !== undefined && (data.altRange.value || data.altRange.total) && data.altRange.units !== "none") {
    ...
    async rollAttack(options = {}) {
        ...
        if (this.actor.type === "npc2" && ["weapon", "equipment"].includes(this.type)) {
            const npcAttackBonusType = itemData.weaponType ?? this.type === "equipment" ? "equipment" : "standard";
            ...
            if (usage.per && ["round", "shot"].includes(usage.per)) {
                ...
                if (game.combat) {
                    Hooks.callAll("consumeCapacityMinute", {
                        actor: this.actor,
                        item: this,
                        value: this._calculateAmmoUsageWithModifiers(usage.value)
                    });
                } else {
        ...
        if (this.actor.type === "npc2" && ["equipment", "weapon"].includes(this.type) && ["mwak", "rwak"].includes(itemData.actionType)) {
            for (const part of parts) {
                if (part.isPrimarySection) {
                    const npcDamageBonusType = itemData.weaponType ?? this.type === "equipment" ? "equipment" : "standard";
